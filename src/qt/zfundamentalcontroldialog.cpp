...
#include "ui_zfundamentalcontroldialog.h"
...
std::list<std::string> ZFundamentalControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFundamentalControlDialog::listMints;

ZFundamentalControlDialog::ZFundamentalControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFundamentalControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZFundamentalControlDialog::~ZFundamentalControlDialog()
{
    delete ui;
}

void ZFundamentalControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFundamentalControlDialog::updateList()
{
...
void ZFundamentalControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFundamentalControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFundamental_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFundamentalControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFundamentalControlDialog::GetSelectedMints()
{
...
void ZFundamentalControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
