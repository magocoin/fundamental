...
#include "zfundamentalcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zFDM ought to be enough for anybody." - Bill Gates, 2017
    ui->zFDMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFDMSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzFDM->setEnabled(false);
        ui->pushButtonMintzFDM->setToolTip(tr("zFDM is currently disabled due to maintenance."));

        ui->pushButtonSpendzFDM->setEnabled(false);
        ui->pushButtonSpendzFDM->setToolTip(tr("zFDM is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFDMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFDM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFDM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFDM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFDM in ") +
...
void PrivacyDialog::on_pushButtonSpendzFDM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFDM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zFDM
        sendzFDM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFDM
    sendzFDM();
}

void PrivacyDialog::on_pushButtonZFundamentalControl_clicked()
{
    ZFundamentalControlDialog* zFDMControl = new ZFundamentalControlDialog(this);
    zFDMControl->setModel(walletModel);
    zFDMControl->exec();
}

void PrivacyDialog::setZFundamentalControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFDMSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFDM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Fundamental Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFDMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFDM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FDMIPCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFDM</b>";
    ...
    // use mints from zFDM selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFundamentalControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZFundamentalControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfundamental selector in case it was used
    ZFundamentalControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFDM Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Fundamental, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFDM Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFDM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFDM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFDM "));
    ui->labelzFDMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzFDMSyncStatus->setVisible(fShow);
}
