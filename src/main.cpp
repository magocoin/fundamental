...
#if defined(NDEBUG)
#error "Fundamental cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zFDM spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zFDM spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zFDM spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 3) {
           nSubsidy = 26666 * COIN;
    } else if (nHeight >= 3&& nHeight <= 8100) {
	          nSubsidy = 1 * COIN;
    } else if (nHeight > 8100 && nHeight <= 26800) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <= 53602 && nHeight > 26800) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <= 80404 && nHeight > 53602) {
        nSubsidy = 12 * COIN;
    } else if (nHeight <= 107205 && nHeight > 80404) {
        nSubsidy = 16 * COIN;
    } else if (nHeight <= 150405 && nHeight > 107205) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 193605 && nHeight > 150405) {
        nSubsidy = 24 * COIN;
    } else if (nHeight <= 280005 && nHeight > 193605) {
        nSubsidy = 28 * COIN;   
    } else if (nHeight <= 366406 && nHeight > 280005) {
        nSubsidy = 20 * COIN;   
    } else if (nHeight <= 716807 && nHeight > 366406) {
        nSubsidy = 15 * COIN;
    } else if (nHeight <= 1067209 && nHeight > 716807) {
        nSubsidy = 10 * COIN;   
    } else if (nHeight <= 1417610 && nHeight > 1067209) {
        nSubsidy = 5 * COIN;	    
    } else {
          nSubsidy = 2 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Fundamental
...
{
    RenameThread("fundamental-scriptch");
...
void RecalculateZFDMSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zFDM supply
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zFDM with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zFDMSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zFDM spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zFDM serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zFDM mints and %d zFDM spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Fundamental: We use certain sporks during IBD, so check to see if they are
